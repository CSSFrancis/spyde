/**
 * index.ts — Electron main process for Ground Crew.
 *
 * Short on purpose. Spawning and supervising the Python sidecar, bootstrapping
 * its uv environment, creating the window and buffering backend messages into
 * it all come from @de/shell-main; what is left is this app's menu and the two
 * message kinds it forwards.
 *
 * If something here starts looking like it should be shared, it probably
 * should — push it down into the shell rather than copying it to the next app.
 */
import { app, ipcMain, Menu, nativeTheme } from 'electron'
import {
  configureShell, createShellWindow,
  startBackend, stopBackend, sendAction, sendFigureEvent, sendResize,
  resolvePythonEnv, registerShellDialogs, type ShellWindow,
} from '@de/shell-main'

configureShell({
  appId: 'groundcrew',
  appName: 'Ground Crew',
  pythonModule: 'de_groundcrew',
  pythonDist: 'de-groundcrew',
})

let win: ShellWindow | null = null
let binaryFrames = 0

// NB no custom figure scheme here, unlike SpyDE. Ground Crew's viewer builds its
// figure with the ESM fully INLINED, so the renderer mounts it in a `srcdoc`
// iframe and nothing has to be served off disk. SpyDE needs the scheme because
// it swaps the inlined bundle for a shared file:// URL (so Chromium reuses the
// V8 code cache across many figure iframes) — worth it only with many figures,
// and a srcdoc frame cannot load it. Should this app ever open figures by the
// dozen, lift SpyDE's scheme into the shell rather than copying it.

app.whenReady().then(async () => {
  // The figures theme off prefers-color-scheme; the chrome is dark, so pin it.
  nativeTheme.themeSource = 'dark'

  Menu.setApplicationMenu(Menu.buildFromTemplate([
    ...(process.platform === 'darwin' ? [{ role: 'appMenu' as const }] : []),
    {
      label: 'Acquisition',
      submenu: [
        { label: 'Start', click: () => sendAction('start_acquisition') },
        { label: 'Stop', click: () => sendAction('stop_acquisition') },
        { type: 'separator' as const },
        { label: 'Single exposure', click: () => sendAction('single_acquisition') },
      ],
    },
    { role: 'viewMenu' },
  ]))

  win = createShellWindow({ mainDir: __dirname })

  // __dirname is out/main → two levels up is the app root, which holds the
  // pyproject.toml `uv run` needs.
  const resolved = await resolvePythonEnv({
    isPackaged: app.isPackaged,
    resourcesPath: process.resourcesPath,
    projectRoot: `${__dirname}/../..`,
    userData: app.getPath('userData'),
  })

  startBackend(resolved.cmd, {
    onMessage: (msg) => {
      // Echo lifecycle messages to this terminal. They otherwise exist only on
      // the PLOTAPP channel, which THIS process consumes — so without the echo
      // backend health is invisible from outside, and an e2e harness has no
      // ready signal to wait on (it watches the app's stdout, not Python's).
      if (msg.type === 'ready' || msg.type === 'error') {
        console.log(`[groundcrew backend] ${msg.type}: ${msg.text ?? ''}`)
      }
      win?.sendToRenderer(msg)
    },
    onBinary: (header, payload) => {
      // A raw PLOTBIN image frame — this is how every pixel update arrives,
      // because the shell's backendProcess turns on APL_BINARY_TRANSPORT. Omit
      // this handler and the runner parses the frames and drops them: the
      // figure mounts, the stats strip ticks, and the image stays on its
      // opening placeholder forever, with nothing logged.
      if (++binaryFrames === 1) {
        console.log(`[groundcrew backend] first binary frame: fig=${header.fig_id} ` +
                    `key=${header.key} bytes=${payload.byteLength}`)
      }
      // Shaped like a state_update so the renderer routes it into the figure by
      // fig_id/key, with `buffer` (bytes) in place of `value` (base64).
      const buf = payload.buffer.slice(
        payload.byteOffset, payload.byteOffset + payload.byteLength)
      win?.sendToRenderer({
        type: 'state_update_binary',
        fig_id: header.fig_id,
        key: header.key,
        header,
        buffer: new Uint8Array(buf),
      })
    },
    onStream: (text, kind) => {
      // Tee to this terminal: the backend's own errors travel the PLOTAPP
      // channel, so without this a dying sidecar dies silently.
      process[kind === 'stderr' ? 'stderr' : 'stdout'].write(text)
      win?.sendToRenderer({ type: 'stream', text, kind })
    },
  }, resolved.cwd)
})

ipcMain.on('groundcrew:action', (_e, action: string, payload: Record<string, unknown>) =>
  sendAction(action, payload))
ipcMain.on('groundcrew:figure-event', (_e, figId: string, eventJson: string) =>
  sendFigureEvent(figId, eventJson))
ipcMain.on('groundcrew:resize', (_e, figId: string, width: number, height: number) =>
  sendResize(figId, width, height))

// Native open/save dialogs, on the shell's channel — Motion needs to load a
// movie stack and write a corrected sum, and a sandboxed renderer cannot ask
// for either by itself.
registerShellDialogs()

app.on('window-all-closed', () => {
  stopBackend()
  if (process.platform !== 'darwin') app.quit()
})
app.on('before-quit', () => stopBackend())
