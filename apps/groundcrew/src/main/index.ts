...
function sendToRenderer(msg: Record<string, unknown>): void {
  ...
  win!.webContents.send('groundcrew:message', msg)
}

function flushPending(): void {
  if (!rendererAlive()) return
  while (pending.length) win!.webContents.send('groundcrew:message', pending.shift())
}
...
function createWindow(): BrowserWindow {
  ...
  w.webContents.on('console-message', (_e, level, message) => {
    if (level >= 1 || message.startsWith('[gc]')) {
      console.log(`[groundcrew renderer] ${message}`)
    }
  })
  ...
  startBackend(resolved.cmd, {
    onMessage: (msg) => {
      ...
      if (msg.type === 'state_update' && ++stateUpdates === 1) {
        console.log(`[groundcrew backend] first state_update: key=${msg.key}`)
      }
      sendToRenderer(msg)
    },
