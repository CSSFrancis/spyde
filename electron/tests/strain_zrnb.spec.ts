...
test('Strain Mapping caret: overlay, component switch, toggle off', async () => {
  ...
  // Wait for the REAL completion signal instead of a fixed sleep — a real
  // LocalCluster's per-worker process spawn can be very slow on this box
  // (60-95s+ even for a tiny dataset; see CLAUDE.md's dask-startup notes), so a
  // fixed guess is unreliable either way. strain_open itself now also self-waits
  // (up to 300s) if Strain is opened before this lands, so this is a courtesy
  // wait, not the only thing standing between "found" and "strain works".
  await ctx.backend.waitForLog('Found', 180_000).catch(() => {})
  ...
  await expect(strainBtn).toBeVisible({ timeout: 15_000 })
