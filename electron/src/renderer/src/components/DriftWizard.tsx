 ...
 *
 * Only `rigid` has a solver. `rigid+affine` is shown LOCKED inside Advanced
 * with the backend's own reason rather than silently falling back — a rigid
 * solve under a caret claiming "rigid+affine" puts a wrong `kind` into the
...
type Method = 'rigid' | 'rigid_affine'
type TabLabel = 'Rigid' | 'Rigid+Affine'
const TABS: readonly TabLabel[] = ['Rigid', 'Rigid+Affine']
const METHOD_OF: Record<TabLabel, Method> = {
  'Rigid': 'rigid', 'Rigid+Affine': 'rigid_affine',
}
const TAB_OF: Record<Method, TabLabel> = {
  rigid: 'Rigid', rigid_affine: 'Rigid+Affine',
}
/** Verbatim from `drift_action._UNAVAILABLE` — the reason the backend gives.
 *
 *  This list is DUPLICATED from the backend, which is a trap worth naming:
 *  implementing a model there while leaving its tab locked here makes the
 *  finished feature unreachable — with every headless test still green,
 *  because none of them can see a disabled tab. If a model is added or
 *  implemented, both ends move. */
  ...
  return (
    ...
    >
      ...
      <Advanced open={advanced} onToggle={() => setAdvanced(v => !v)}>
        ...
        />
        {/* The stub is locked, so this names it rather than waiting for a
