...
options. Information overload."**
    ...
    drift_set_method  rigid | rigid+affine | non-rigid (lives in Advanced now)
...
#: the other two are declared so a host can render the choice, and both fall
#: back to ``rigid`` with an explicit status rather than silently doing
#: something the user did not ask for. They live inside Advanced now — three
#: visible tabs for two choices that do not work was two thirds of a control
#: row spent on nothing (§0.9a).
